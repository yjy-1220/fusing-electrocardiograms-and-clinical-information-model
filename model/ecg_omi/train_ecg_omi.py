...
from model.common.data import load_npz, split_data
...
def parse_args():
    ...
    p.add_argument("--data", type=str, default=None, help="labeled OMI data (.npz)")
    p.add_argument("--pretrain-data", type=str, default=None, help="unlabeled ECG .npz (ecg field only)")
    ...
    p.add_argument("--seq-len", type=int, default=250, help="500 Hz x 500 ms median beat")
...
def main():
    ...
    # ---- 1. Data loading ----
    if not cfg.data:
        raise SystemExit("Provide --data pointing to a labeled .npz file.")
    data = load_npz(cfg.data)
    ...
    if not cfg.no_pretrain:
        if cfg.pretrain_ckpt:
            ...
            unlabeled_ecg = load_npz(cfg.pretrain_data)["ecg"]
        else:
