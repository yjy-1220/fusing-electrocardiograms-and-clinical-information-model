...
def run_cross_validation(ecg, tokenized_texts, labels, cfg, device, pretrain_ckpt):
    ...
    for k, (tr, va) in enumerate(skf.split(np.zeros(len(labels)), labels), start=1):
        ...
        auc, _ = run_finetuning_multimodal(model, tr_loader, va_loader, cfg, device, tag=f"cv-fold-{k}")
        aucs.append(auc)
        del model
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
    mean_auc = float(np.mean(aucs))
    print(f"[CV] {cfg.cv_folds}-fold AUROC: {[f'{a:.4f}' for a in aucs]} | mean {mean_auc:.4f}")
    return mean_auc
