"""Multimodal data: ECG records + clinical text .csv loading / alignment / splitting / tokenization / Dataset.

Generic record loading lives in model.common.ecg_io; split_data in model.common.data.
"""
import numpy as np
import torch
...
def align_ecg_text(ecg_data: dict, text_df, pid_col=None):
    ...
    Labels are taken from the ECG source (DSA gold standard) when it carries any;
    otherwise the csv labels are used, and when both are present the csv is only
    ...
    if labels is not None and len(ecg) != len(labels):
        raise SystemExit(f"ecg and labels length mismatch: {len(ecg)} vs {len(labels)}")
    if labels is None:
        print("[Align] ECG source carries no labels; using the labels from the text csv.")
    ...
    if pid_col is None:
        ...
        out_labels = labels if labels is not None else text_labels
        return ecg, np.asarray(out_labels, dtype=np.int64), texts, None
    ...
    for i, k in enumerate(ecg_keys):
        ...
        # ECG-side labels are the DSA gold standard; fall back to the csv when the
        # ECG source (e.g. a directory of raw records) has none of its own.
        y_ecg = int(labels[i]) if labels is not None else y_csv
        if y_csv != y_ecg:
            mismatch += 1
        idx_ecg.append(i)
        out_texts.append(t)
        out_labels.append(y_ecg)
    if dropped:
        print(f"[Align] {dropped} ECG records have no matching text and were dropped.")
    if mismatch:
        print(f"[Align] Warning: {mismatch} records have text CSV labels inconsistent with the ECG labels; the ECG side (DSA gold standard) takes precedence.")
    if not idx_ecg:
